...
class Helicorder(plt.Figure):
    ...
    def plot(self, *args, **kwargs):
        ...
        if self.background_color:
            ...
            ax = self.add_subplot(1, 1, 1, **axis_facecolor_kwargs)
        else:
            ax = self.add_subplot(1, 1, 1)
        # Adjust the subplots
        self.subplots_adjust(left=self.subplots_adjust_left,
        ...
        self.axes[0].grid(color=self.grid_color,
                              linestyle=self.grid_linestyle,
                              linewidth=self.grid_linewidth)
        self.axes[0].yaxis.grid(False)
        # Set the title of the plot.
        if self.title is not None:
            self.suptitle(self.title, fontsize=self.title_size)
