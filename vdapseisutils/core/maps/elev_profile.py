...
class TopographicProfile:
    ...
    def __init__(self, points, source="opentopo", resolution=500.0, max_n=100):
        ...
        # resolution is the frequency of evelation samples along the line (in meters)
        max_n = int(max_n)
        ...
        self.source = source  # where to get data ('optentopo', 'pygmt', 'file')
        try:
            data = download_profile(self.points[0], self.points[1], n=self.n) # returns dict w lat, lon, d, elev, elev_m, elev_km
            self.elevation = data["elev_m"]  # elevation in meters
            self.distance = data["d"] # horizontal distance along line in meters
        except:
            print("There was a problem downloading elevation data. Elevation and distance arrays are empty.")
            self.elevation = np.array([])
            self.distance = np.array([])
...
def test_topoprofile_class():
    ...
    tp = TopographicProfile((-8.169148, 115.283046), (-8.579110, 115.819991), resolution=500.0)
    tp.plot()
