"""
Waveform stream identifiers extending ObsPy's :class:`~obspy.core.event.base.WaveformStreamID`.

``VStreamID`` is the canonical type. The name ``waveID`` is a deprecated alias for
:class:`VStreamID` (same object); new code should use ``VStreamID``.
"""
...
class VStreamID(WaveformStreamID):
    def __init__(self, *args, order="nslc", sep=".", blank="", **kwargs):
        """
        Initializes the VStreamID object.
    ...
    def from_dict(self, d):
        """
        Read VStreamID from a dictionary. Understands keys:
    ...
    def scnl2nslc(scnl, sep=".", new_sep="."):
        scnl_id = VStreamID(scnl, order="scnl", sep=sep)
...
# Deprecated name for VStreamID; kept for backward compatibility with legacy imports.
waveID = VStreamID

__all__ = ["VStreamID", "parse_wave_id", "waveID"]
