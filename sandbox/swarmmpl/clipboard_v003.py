# Revert to this copy if v004 goes sideways
...
# TODO Remove unnecssary ticklabels
...
class Clipboard(plt.Figure):
    ...
    def _plot_clipboard(self):
        ...
        for i, tr in enumerate(st):
            ...
            self.axes[axn].set_xticklabels(labels)

            # Secondary axis - specgram axis
            xlim = [xlim[0] / self.spectrogram["samp_rate"], xlim[1] / self.spectrogram["samp_rate"]]
            self.add_subplot(gs[axn + 1])
            tr.g_kwargs(axes=self.axes[axn + 1], **self.spectrogram)
            self.axes[axn + 1].set_xlim(xlim)
            ticks, labels, xlabel = __define_t_ticks__(self.data_extent, self.axes[axn + 1].get_xlim(), tick_type=self.tick_type)
            self.axes[axn + 1].set_xticks(ticks)
            self.axes[axn + 1].set_xticklabels(labels)
            self.axes[axn + 1].set_xlabel(xlabel)
