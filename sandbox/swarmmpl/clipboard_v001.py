...
class Clipboard(plt.Figure):
    ...
    def __init__(self, st, *args, mode="wg", taxis="datetime",
                 figsize=None,
        ...
        self.__make_all_axes()

    def __make_all_axes(self):
    ...
    def plot(self):
        ...
        for i in range(self.ntr):
            ...
            if self.mode == "wg" or self.mode == "g":
                ...
                axP = fig.add_subplot(gs[i*self.nax+a])  # Create subplot for g_kwargs
                ...
                # )
                axP = tr.g_kwargs(axes=axP, **self.spectrogram_settings)
            ...
            # Remove middle axes
            if len(fig.get_axes()) >= 2:  # if len>2 bc 1 st will produce two axes
...
def plot_triggers(triggers, cft, st, cftlim=None,
    ...
    """SWARMWG Plots waveform/g_kwargs pairs for Stream objects with multiple Traces
    ...
    KWARGS
    cftlim      : list-like : 2 element list of Y-axis limits for g_kwargs (Hz)
                  Default:[1,10]
    ...
    # ??? settaxis( fig, n=2, minticks=3, maxticks=7 ) # n is how many plots per st
    """
    The swarmwg() function makes two axes for each waveform - a waveform axis (w)
    and a g_kwargs axis (g).
