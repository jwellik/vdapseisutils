...
# TODO BIG PROBLEM if axis extent is not the same as data extent!
    ...
    def load_streams(self, st, mode="wg", **kwargs):
        ...
        for i in range(self.ntr):
            ...
            if self.mode == "wg" or self.mode == "g":
                ...
                self.subplots.append(self.add_subplot(gs[axn]))  # Create subplot for g_kwargs

                self.subplots[axn].yaxis.set_ticks_position('right')
                tr.g_kwargs(axes=self.subplots[axn], **self.spectrogram)
