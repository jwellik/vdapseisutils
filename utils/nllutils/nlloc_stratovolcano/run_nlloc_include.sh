...
Vel2Grid run/nlloc_stratovolcano_include.in
echo

echo "Create and visualize the model grid"
Grid2GMT run/nlloc_stratovolcano_include.in model/layer.P.mod gmt/ V G 1 0 1 401
#${PS_VIEWER} gmt/layer.P.mod.VG.ps &
echo

echo "Generate and view the travel-time and take-off angle grids "
Grid2Time run/nlloc_stratovolcano_include.in
#echo
#echo "Visualize P travel-time grid"
#Grid2GMT run/nlloc_stratovolcano_include.in time/layer.P.AK_RC01_--.time gmt/ V G 0 0 0 401
#${PS_VIEWER} gmt/layer.P.AK_RC01_--.time.VG.ps &
#echo "Visualize P take-off angles grid"
#Grid2GMT run/nlloc_stratovolcano_include.in time/layer.P.AK_RC01_--.angle gmt/ V G 0 0 0 401
#${PS_VIEWER} gmt/layer.P.AK_RC01_--.angle.VG.ps &
echo

echo "Generate some synthetic arrival times "
Time2EQ run/nlloc_stratovolcano_include.in
more obs_synth/synth.obs
echo

echo "Do the event Location "
NLLoc run/nlloc_stratovolcano_include.in

echo "Plot the combined locations with GMT"
LocSum loc/volcano.sum.grid0.loc 1 loc/volcano "loc/volcano.*.*.grid0.loc"
Grid2GMT run/nlloc_stratovolcano_include.in loc/volcano gmt/ L E101

echo "Plot each location w GMT"
Grid2GMT run/nlloc_stratovolcano_include.in "loc/volcano.*.*.grid0.loc" gmt/ L E111
Grid2GMT run/nlloc_stratovolcano_include.in "loc/volcano.*.*.grid0.loc" gmt/ L S
...
#java net.alomax.seismicity.Seismicity loc/volcano.*.*.grid0.loc.hyp
