...
class DataSource:
    ...
    def __init__(self, ds_type, ds_string, filepattern='*'):
        ...
        # else datasource is a server/Client >>> createClient()
            ...
            # DataSource is some other type of server
                ...
                server, port = ds_string.split(':'); port = int(port)
    ...
    def getWaveforms(self, nslc_list, tstart, tend,
        ...
        if self.ds_type.lower() == 'filelist'.lower():
            ...
            st = self.get_waveforms_from_files(nslc_list, tstart, tend, create_empty_trace=create_empty_trace, verbose=verbose)

        elif self.ds_type.lower() == 'client'.lower():

            st = self.get_waveforms_from_client(nslc_list, tstart, tend, create_empty_trace=create_empty_trace, verbose=verbose)
